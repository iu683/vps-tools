...
while true; do
  ...
  # --- 双列菜单 (使用Tab制表符对齐) ---
  printf "${YELLOW} 1.${RESET} 更新源\t\t\t\t${YELLOW}15.${RESET} 国内 EZRealm\n"
  printf "${YELLOW} 2.${RESET} 安装 curl\t\t\t\t${YELLOW}16.${RESET} V0 哪吒\n"
  printf "${YELLOW} 3.${RESET} 安装 unzip\t\t\t\t${YELLOW}17.${RESET} 一点科技\n"
  printf "${YELLOW} 4.${RESET} 卸载哪吒探针\t\t\t${YELLOW}18.${RESET} Sub-Store Docker\n"
  printf "${YELLOW} 5.${RESET} v1 哪吒关闭SSH\t\t\t${YELLOW}19.${RESET} 宝塔面板\n"
  printf "${YELLOW} 6.${RESET} v0 哪吒关闭SSH\t\t\t${YELLOW}20.${RESET} 1panel 面板\n"
  printf "${YELLOW} 7.${RESET} DDNS 脚本\t\t\t\t${YELLOW}21.${RESET} WEBSSH Docker\n"
  printf "${YELLOW} 8.${RESET} 安装 HY2\t\t\t\t${YELLOW}22.${RESET} 宝塔开心版\n"
  printf "${YELLOW} 9.${RESET} 安装 3XUI\t\t\t\t${YELLOW}23.${RESET} IP 解锁 (IPv4)\n"
  printf "${YELLOW}10.${RESET} 老王工具箱\t\t\t\t${YELLOW}24.${RESET} IP 解锁 (IPv6)\n"
  printf "${YELLOW}11.${RESET} 科技 lion\t\t\t\t${YELLOW}25.${RESET} 网络质量 (IPv4)\n"
  printf "${YELLOW}12.${RESET} WARP\t\t\t\t\t${YELLOW}26.${RESET} 网络质量 (IPv6)\n"
  printf "${YELLOW}13.${RESET} SNELL\t\t\t\t\t${YELLOW}27.${RESET} NodeQuality 脚本\n"
  printf "${YELLOW}14.${RESET} 国外 EZRealm\n"

  echo -e "${BOLD}${BLUE}-------------------------------------------------------------------${RESET}"
  printf "${GREEN}28.${RESET} 设置快捷指令 'M'\t\t\t${RED} 0.${RESET} 退出工具箱\n"
  echo -e "${BOLD}${BLUE}===================================================================${RESET}"
