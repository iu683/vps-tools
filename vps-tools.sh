...
print_menu() {
    clear
    rainbow_line "┌───────────────────────────────────────────────┐"
    rainbow_line "│        🌈 服务器工具箱（双列版） 🌈             │"
    rainbow_line "├───────────────────────────────────────────────┤"
    
    local total=${#MENU_ITEMS[@]} half=$(( (total + 1) / 2 ))
    for ((i=0; i<half; i++)); do
        IFS="|" read -r num1 name1 cmd1 <<< "${MENU_ITEMS[i]}"
        if ((i + half < total)); then
            IFS="|" read -r num2 name2 cmd2 <<< "${MENU_ITEMS[i + half]}"
            printf "│ \033[1;33m%2s\033[0m. %-22s \033[1;33m%2s\033[0m. %-22s │\n" \
                "$num1" "$name1" "$num2" "$name2"
        else
            printf "│ \033[1;33m%2s\033[0m. %-22s %-27s │\n" \
                "$num1" "$name1" ""
        fi
    done
